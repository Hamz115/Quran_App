// Static Quran data: page-to-surah mapping and Arabic surah names.
// Ported from quran_frontend/src/data/quranPages.ts and QuranReader.tsx.

const int totalPages = 604;

/// pageStarts[pageNumber - 1] = [surahNumber, ayahNumber] of first ayah on that page.
const List<List<int>> pageStarts = [
  [1, 1],    // Page 1
  [2, 1],    // Page 2
  [2, 6],    // Page 3
  [2, 17],   // Page 4
...
const Map<int, String> surahNamesArabic = {
...
int getPageNumber(int surahNum, int ayahNum) {
  for (int i = pageStarts.length - 1; i >= 0; i--) {
    final surah = pageStarts[i][0];
    final ayah = pageStarts[i][1];
    if (surahNum > surah || (surahNum == surah && ayahNum >= ayah)) {
      return i + 1;
    }
  }
  return 1;
...
// ============ JUZ BOUNDARIES (with ayah precision) ============

/// Juz boundary with ayah-level precision (Madani Mushaf standard).
class JuzBoundary {
  final int juz;
...
List<int> getSurahsOnPage(int pageNumber) {
  ...
  final startSurah = pageStarts[pageNumber - 1][0];
  final endSurah = pageNumber < totalPages
      ? pageStarts[pageNumber][0]
      : 114;
